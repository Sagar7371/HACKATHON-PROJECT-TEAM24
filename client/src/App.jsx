...
function App() {
  ...
  useEffect(() => { setPage(1); }, [category, search, teachQuery, wantsQuery, location, format, level, availability]);
  useEffect(() => { let cancelled = false; setSkillsLoading(true); setSkillsError(''); getSkills({ category, search, teach: teachQuery, wants: wantsQuery, location, format, level, availability, page, limit: 8 }).then((result) => { if (!cancelled) { setSkills((current) => page === 1 ? result.items : [...current, ...result.items]); setHasMore(result.hasMore); } }).catch(() => { if (!cancelled) { setSkills([]); setSkillsError('We could not load the exchange board.'); } }).finally(() => { if (!cancelled) setSkillsLoading(false); }); return () => { cancelled = true; }; }, [category, search, teachQuery, wantsQuery, location, format, level, availability, page]);
  ...
  const sortedSkills = [...skills].sort((first, second) => sort === 'rating' ? second.teacher.rating - first.teacher.rating : sort === 'newest' ? String(second._id).localeCompare(String(first._id)) : 0);
